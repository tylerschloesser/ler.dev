...
test('deep routes fall back to index.html', async ({ page }) => {
  const response = await page.goto('/some/deep/route')
  expect(response?.status()).toBe(200)
  await expect(page.getByRole('heading', { level: 1 })).toHaveText('Hello world')
})

test('missing assets are not masked', async ({ request }) => {
  const response = await request.get('/nope.js')
  expect(response.status()).not.toBe(200)
})

test('ler.dev redirects to ty.ler.dev', async ({ request }) => {
...
test('CloudWatch RUM ingests events', async ({ page }) => {
